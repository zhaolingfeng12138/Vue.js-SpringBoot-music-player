<template>
	<view class="lyric-main" :style="{
		...
		<scroll-view id="lyric-show" class="lyric-show" scroll-with-animation="true" :scroll-y="true"
			:scroll-into-view="showLyricId" @scroll="lyricScroll" :style="{
		top:scrollView.top,
		width:cuAreaStyle.width,
		height:scrollView.height}">
			<view class="lrc-item" v-for="(v, i) in mLyrics.lrcs" :key="i" @tap="select(v,i)"
				@longpress="itemLongpress(v,i)" @touchstart="textTouchstart" @touchcancel="textTouchEnd"
		...
		<view class="center-view"
			:style="{top:centerLineTop,display:showCenterView?'flex':'none',fontSize:cuCenterStyle.fontSize,height:cuCenterStyle.height}">
			<!--image v-if="cuCenterStyle.btnImg" class="center-btn" @click="centerBtnClick" :src="cuCenterStyle.btnImg"
				:style="{width:cuCenterStyle.height,height:cuCenterStyle.height}"></image>
			<view v-else class="center-btn" @click="centerBtnClick">{{cuCenterStyle.btnText}}</view>
			<view class="center-line" :style="{height:cuCenterStyle.lineHeight,backgroundColor:cuCenterStyle.color}">
			</view>
			<view class="center-time">{{centerTime}}</view-->
		</view>
		<view class="selectControl" v-if="selectStatus"
			:style="{color:cuSelectControlStyle.color,backgroundColor:cuSelectControlStyle.backgroundColor,fontSize:cuSelectControlStyle.itemFontSize}">
			<view class="selectAll" @click="selectAll"
				:style="{borderRadius:cuSelectControlStyle.itemBorderRadius,backgroundColor:cuSelectControlStyle.itemBackgroundColor}">
				{{selectAllStatus ? '全不选' : '全选'}}</view>
			<view class="copy" @click="copyLyric"
				:style="{borderRadius:cuSelectControlStyle.itemBorderRadius,backgroundColor:cuSelectControlStyle.itemBackgroundColor}">
				复制歌词</view>
