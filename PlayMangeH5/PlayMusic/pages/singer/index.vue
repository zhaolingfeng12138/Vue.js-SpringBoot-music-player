<template>
	<view class="singer-container">
		<cu-custom bgColor="#fff" :isBack="false">
			<view slot="content" style="color: #000">热门歌手</view>
		</cu-custom>
		<scroll-view scroll-y class="indexes" :scroll-into-view="'indexes-' + listCurID" :style="[{ height: height }]"
			...
			<view class="cu-list page-background menu-avatar no-padding">
				<view class="cu-item bg-white" v-for="val in list" :key="val.id" @click="searchDetailSinger(val)">
					<view class="cu-avatar round lg" :style="'background-image:url(' + val.pic + '?param=100y100)'">
					</view>
					<view class="content">
		...
		<!--view class="indexBar" :style="[{ height: 'calc(' + height + ' - 50px)' }]">
			<view class="indexBar-box" @touchstart="tStart" @touchend="tEnd" @touchmove.stop="tMove">
				<view class="indexBar-item" :class="{ active: item.name === listCur }" v-for="(item, index) in list"
	...
	export default {
		...
		methods: {
			...
			searchDetailSinger(val) {
				uni.navigateTo({
					url: '../singerPlayList/index?id=' + val.id + "&name=" + val.name
				});
			},
			//获取文字信息
			getCur(e) {
