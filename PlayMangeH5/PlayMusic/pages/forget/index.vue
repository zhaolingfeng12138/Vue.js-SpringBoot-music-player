		...
		<view class="absolve-wrapper">
			...
			<view class="login-main">
				<view class="input-wrapper">
					...
					<input placeholder="请输入手机号" v-model="phone" maxlength="name" />
				</view>
				<view class="input-wrapper">
					<i class="iconfont icon-password icon"></i>
					<input v-if="isText" type="text" placeholder="请输入密码" v-model="password1" />
...
<script>
	...
	export default {
		...
		methods: {
			//忘记
			getUserInfo() {
				if (this.phone.length == "" || this.password1.length == "" || this.password2.length == "") {
					uni.showToast({
						icon: 'none',
						position: 'bottom',
						title: '用户名或者密码不能为空'
					...
					if(this.password1 == this.password2){
						...
						uni.request({
							url: Vue.prototype.url + 'updateUsersPassword',
							data: data,
							success: (res) => {
