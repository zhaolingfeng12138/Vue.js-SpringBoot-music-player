<template>
	<div class="playList-container">
		<cu-custom bgColor="#fff" :isBack="true">
			<view slot="content" style="color: #000">歌单</view>
		</cu-custom>
		<scroll-view scroll-x class="bg-white nav" scroll-with-animation :scroll-left="scrollLeft">
			<view class="cu-item" :class="item.name == currentName ? 'basic-icon-color cur' : ''"
				v-for="(item, index) in tagList" :key="index" @click="tabSelect(item, index)">
				{{ item.name }}
			</view>
		</scroll-view>
		<scroll-view scroll-y scroll-with-animation @scrolltolower="reachBottom()"
			:style="{ height: height, 'margin-top': '20rpx' }">
			<!--box-title title="热门专辑" style="margin-top: 10px;"></box-title>
			<view class="tower-swiper" @touchmove="TowerMove" @touchstart="TowerStart" @touchend="TowerEnd">
				<view class="tower-item" :class="item.zIndex == 1 ? 'none' : ''" v-for="(item, index) in swiperList"
					...
					<view class="swiper-item">
						<image :src="item.pic + '?param=200y200'" mode="aspectFill" class="swiper-item-img">
							<view class="title">{{ item.name }}</view>
						</image>
					</view>
				</view>
			</view-->
			<box-title title="热门专辑" style="margin-top: 30px;"></box-title>
			<div class="musicList">
				<view class="list-item" v-for="item in recommendList" :key="item.id">
					...
					<view class="bg-img flex align-end list-item-image" :style="'background-image:url(' + item.pic + '?param=230y230)'" @click="toPlayListDetail(item)">
	...
	export default {
		...
		methods: {
					...
					success: (res) => {
						if (res.data.code == "200") {
							for(var i=0; i < res.data.data.length; i++){
								res.data.data[i].playCount = this.getRandomInt(500, 1000);
			...
			toPlayListDetail(val) {
				uni.navigateTo({
					url: '../songDetail/index?id=' + val.id + "&name=" + val.name
				});
			}
		}
	};
